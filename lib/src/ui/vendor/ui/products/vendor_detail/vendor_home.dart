...
class _VendorHomeState extends State<VendorHome> {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(onRefresh: () async {
      await widget.vendorDetailsModel.getDetails(context);
      return;
    ...
    for (var i = 0; i < vendorDetails.blocks.length; i++) {
      ...
      if (vendorDetails.blocks[i].blockType == 'product_block' &&
        ...
        list.add(ProductScroll(
            products: vendorDetails.blocks[i].products,
            context: context,
            title: vendorDetails.blocks[i].title,
            viewAllTitle: AppStateModel().blocks.localeText.viewAll,
            filter: filter));
      }
