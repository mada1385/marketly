...
class VendorDetailStateModel extends Model {
  ...
  void getDetails(BuildContext context) async {
    ...
    final response = await apiProvider.post(
        '/wp-admin/admin-ajax.php?action=mstore_flutter-vendor_details&lang=${myLocale.languageCode}',
        filter);
  ...
  Future<bool> loadMore() async {
    ...
    List<Product> moreProducts = await apiProvider.fetchProductList(
      filter,
    );
    if (moreProducts.length != 0) {
      vendorDetails.recentProducts.addAll(moreProducts);
      hasMoreItems = moreProducts.length > 965;
      notifyListeners();
    }
  }
